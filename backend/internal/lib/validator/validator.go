...
func mapValidationErrors(ctx context.Context, errs lib.ValidationErrors) ValidationErrorMessages {
	...
	for _, err := range errs {
		...
		case "password_character":
			message = "must include at least one lowercase letter, one uppercase letter, one numeric digit, and one special character (e.g., !@#$%^&*)."
		case "postal_code":
			message = "must be in the xxx-xxxx format."
		case "prefecture":
			message = "is not a valid prefecture."
		default:
			slogger.WarnCtx(ctx, "failing back to default error message", "ActualTag", err.ActualTag(), "Tag", err.Tag(), "Param", err.Param())
			message = "is invalid."
		}
