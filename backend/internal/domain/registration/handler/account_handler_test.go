		...
		},
		{
			...
			assert: func(t *testing.T, got *connect.Response[registrationv1.SignUpResponse], err error) {
				...
				if errMsg, ok := detail.(*commonv1.BadRequestError); ok {
					...
					require.Equal(t, i18n.MustJapaneseMessage(i18n.Config{MessageID: "registration.usecase.common.error.email_already_exists"}), errMsg.FieldViolations[0].Descriptions[0])
				} else {
