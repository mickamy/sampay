...
func (uc *getMe) Do(ctx context.Context, input GetMeInput) (GetMeOutput, error) {
	...
	if err := uc.reader.ReaderTransaction(ctx, func(tx database.ReaderTransactional) error {
		var err error
		m, err = uc.userRepo.WithTx(tx.ReaderDB()).Get(ctx, contexts.MustAuthenticatedUserID(ctx), userRepository.UserJoinProfileAndImage, userRepository.UserPreloadLinksQRCodeAndDisplayAttributes)
		if err != nil {
