...
func (uc *getUser) Do(ctx context.Context, input GetUserInput) (GetUserOutput, error) {
	...
	if err := uc.reader.ReaderTransaction(ctx, func(tx database.ReaderTransactional) error {
		var err error
		m, err = uc.userRepo.WithTx(tx.ReaderDB()).FindBySlug(ctx, input.Slug, userRepository.UserJoinProfileAndImage, userRepository.UserPreloadLinksQRCodeAndDisplayAttributes)
		if err != nil {
